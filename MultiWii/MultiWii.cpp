//Start as Flying Wing VTOL Tail Sitter
/*
...
//VTOL
uint8_t VTOL_CURRENT_MODE = VTOL_MODE_HOV;
//uint8_t VtolTransfor = 0;
//int16_t Transfor_THR = 0;
//int16_t Transfor_PITCH_Target;
int16_t rcCommand_temp;
...
void setup() {
  ...
  f.VTOL_FFF = 0;
  f.VTOL_PAS = 0;
  VTOL_CURRENT_MODE = VTOL_MODE_HOV;
  #ifdef DEBUGMSG
...
void loop () {
  ...
  if ((int16_t)(currentTime-rcTime) >0 ) { // 50Hz
    ...
    //VTOL
    //Update VTOL fly mode
    if(rcOptions[BOXHOV])
    {
      if(VTOL_CURRENT_MODE != VTOL_MODE_HOV)
      {
        f.VTOL_HOV = 1;
        f.VTOL_FFF = 0;
        f.VTOL_PAS = 0;
      ...
      else if(VTOL_CURRENT_MODE == VTOL_MODE_HOV)
      {
        //nothing happen
      }
    }
    ...
    {
      if(VTOL_CURRENT_MODE != VTOL_MODE_FFF)//HOV to FFF
      {
        f.VTOL_HOV = 0;
        f.VTOL_FFF = 1;
        f.VTOL_PAS = 0;
        VTOL_CURRENT_MODE = VTOL_MODE_FFF;
      }
      else if(VTOL_CURRENT_MODE == VTOL_MODE_FFF)
      {
        //nothing happen
      }
    }
    else if(rcOptions[BOXPAS])
    {
      if(VTOL_CURRENT_MODE != VTOL_MODE_PAS)//HOV to FFF
        {
          f.VTOL_HOV = 0;
          f.VTOL_FFF = 0;
          f.VTOL_PAS = 1;
          VTOL_CURRENT_MODE = VTOL_MODE_PAS;
        }
    }
  ...
  //VTOL
  //When switch to FFF mode, exchange roll and yaw's value
  if((f.VTOL_FFF)||(f.VTOL_PAS))
  {
    rcCommand_temp = rcCommand[ROLL];
    rcCommand[ROLL] = rcCommand[YAW];
    rcCommand[YAW] = -rcCommand_temp;
  }
  ...
  */
  ...
  if(f.VTOL_PAS)
    PTerm = mul(rc,conf.pid[YAW].P8)>>6;  //In pas mode, YAW movement is not gyro related.
  else  
